    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          //TODO: Convert way points from global to car coordinate system
          ...
          for(int i=0; i<ptsx.size();  i++)
          {
            ...
            //std::cout<<"ptsx_car["<<i<<"] ="<<ptsx_car[i];
            //std::cout<<"ptsy_car["<<i<<"] ="<<ptsy_car[i];
          }
          ...
          //Calculate cte
          double cte = polyeval(coeffs, 0);//Calculate CTE at x = 0

          //calculate the orientation error
          double epsi = - atan(coeffs[1] + 2*coeffs[2]*px_car + 3*coeffs[3]*pow(px_car, 2));
          ...
          */
          ...
          state << px_car, 0, psi, v, cte, epsi;

          vector<double> vars = mpc.Solve(state, coeffs);

          std::vector<double> index_vals = mpc.getStartIndexVal();


          double steer_value = vars[index_vals[6]];
          double throttle_value = vars[index_vals[7]];

          json msgJson;
          // NOTE: Remember to divide by deg2rad(25) before you send the steering value back.
          // Otherwise the values will be in between [-deg2rad(25), deg2rad(25] instead of [-1, 1].
          msgJson["steering_angle"] = -steer_value;//- because negate steer angle
          ...
          for(int i=0; i<9; i++)
          {
            ...
            mpc_y_vals.push_back(vars[10+i]);
          }
