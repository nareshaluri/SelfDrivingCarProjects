...
double ref_v = 50 ;//* 0.44704;//40 mph
...
std::vector<double> MPC::getStartIndexVal()
{
  std::vector<double> idx_list;
  idx_list.push_back(x_start);
  idx_list.push_back(y_start);
  idx_list.push_back(psi_start);
  idx_list.push_back(v_start);
  idx_list.push_back(cte_start);
  idx_list.push_back(epsi_start);
  idx_list.push_back(delta_start);
  idx_list.push_back(a_start);
  return idx_list;
}
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for(int i = 0; i<n_vars; i++)
  {
    ret_val.push_back(solution.x[i]);
  }
  return ret_val;
}
